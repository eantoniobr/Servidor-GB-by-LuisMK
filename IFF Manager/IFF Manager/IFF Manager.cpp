...
int main(int argc, char* argv[]) {
	...
	cmd.ConsoleSetTitle("IFF MANAGER");
	cmd.Log();
	...
	std::cout << "0. Fecha o app.\n"
		<< "1. Gera Lista itemBase Ativo no shop.\n"
...
test:try {
		...
		if (escolha == 1) {
			...
			_smp::message_pool::getInstance().push(new message(table, CL_FILE_LOG_AND_CONSOLE));
			...
			_smp::message_pool::getInstance().push(new message(insert_item_10_curva_str, CL_FILE_LOG_AND_CONSOLE));
			...
			_smp::message_pool::getInstance().push(new message(json, CL_FILE_LOG_AND_CONSOLE));
			...
			for (auto& el : cards) {
				...
				if (IFF::Card::T_PACK == type) {
					...
					_smp::message_pool::getInstance().push(new message(std::string("Card Pack:\nTypeid: ") + std::to_string(el.second._typeid) + " (0x" 
							...
							+ "\nDate Start: " + _formatDate(el.second.date.date[0]) + "\t-\tEnd: " + _formatDate(el.second.date.date[1]), CL_FILE_LOG_AND_CONSOLE));
			...
			// Enable PS
			_smp::message_pool::getInstance().push(new message("Enable Count: " + std::to_string(enable_ps.size()), CL_FILE_LOG_AND_CONSOLE));

			for (auto& el : enable_ps)
				_smp::message_pool::getInstance().push(new message(el, CL_FILE_LOG_AND_CONSOLE));

			// Disable PS
			_smp::message_pool::getInstance().push(new message("Disable Count: " + std::to_string(disable_ps.size()), CL_FILE_LOG_AND_CONSOLE));
			...
			_smp::message_pool::getInstance().push(new message(insert_db, CL_FILE_LOG_AND_CONSOLE));
			...
			for (auto& el : iff.getGrandPrixData()) {
				...
				_smp::message_pool::getInstance().push(new message("GP[TYPEID: 0x" + hex_util::ltoaToHex(el.second._typeid) 
						...
						+ std::to_string(el.second.bot.score_max) + "]", CL_FILE_LOG_AND_CONSOLE));
			}
		}
