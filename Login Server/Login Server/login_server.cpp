...
void login_server::requestLogin(player& _session, packet *_packet) {
	...
	packet p;
	uint8_t login_falied = 0;
	//try
	//{
	//	if (m_manutenction)
	//	{
	//		throw exception("[login_server::requestLogin][Error] servidor em manutencao, use o comando 'start' para reiniciar o server !", STDA_MAKE_ERROR(STDA_ERROR_TYPE::LOGIN_SERVER, 253, 0));
	//	}
	//}
	//catch (exception& e) 
	//{
	//	if (STDA_ERROR_CHECK_SOURCE_AND_ERROR(e.getCodeError(), STDA_ERROR_TYPE::LOGIN_SERVER, 253/*Invalid ID*/)) {

	//		// Invalid ID
	//		packet_func::pacote001(p, &_session, 15/*Invlid ID*/);
	//		packet_func::session_send(p, &_session, 1);
	...
	try {
		...
		try {
			if (pass_md5.size() < 32)
				throw exception("[login_server::requestLogin][Error] PASS(" + pass_md5
					+ ") senha nao contem 32 caracteres. bug ou hacker !", STDA_MAKE_ERROR(STDA_ERROR_TYPE::LOGIN_SERVER, 252, 0));
		}catch (exception& e) {
		...
		if (!haveBanList(_session.m_ip, mac_address, !mac_address.empty())) {	// Verifica se está na list de ips banidos
...
bool login_server::checkCommand(std::stringstream& _command) {
	...
	else if (!s.empty() && s.compare("start") == 0) {
...
bool login_server::checkPacket(session& _session, packet *_packet) {
	...
	if (_session.m_check_packet.checkPacketId(_packet->getTipo()) && _session.m_check_packet.incrementCount() >= CHK_PCKT_COUNT_LIMIT) {
