...
uint32_t unit_auth_server_connect::getOID() {
	return m_session.m_oid;
}
...
void unit_auth_server_connect::onHeartBeat() {
	...
	try {
		...
		//_smp::message_pool::getInstance().push(new message("[unit_auth_server_connect::onHearBeat][ErrorSystem] " + e.getFullMessageError(), CL_ONLY_CONSOLE));
	}
}
