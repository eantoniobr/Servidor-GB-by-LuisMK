...
#endif
	  m_shutdown(nullptr), m_Bot_TTL(10000u), m_chat_discord(false)/*Padrão ativado*/ {
...
#endif
							_smp::message_pool::getInstance().push(new message("Players online: " + std::to_string(m_session_manager.numSessionConnected()), CL_FILE_LOG_AND_CONSOLE));
...
void server::accept_completed(SOCKET *_listener, DWORD dwIOsize, myOver *lpBuffer, DWORD _operation) {
	...
	try {
		...
		if (dwIOsize == 0/*Não uso o que recebe dados no accept, e uso esse valor para flag de erro para deletar o player que caiu*/) {
			...
			_smp::message_pool::getInstance().push(new message("[server::acceptEx][Log] Players Online: " + std::to_string(m_session_manager.numSessionConnected()), CL_FILE_LOG_AND_CONSOLE));
