		...
		*/
		...
		struct Mascot : public Base {
			...
			struct Mensagem {
				...
				unsigned char flag;					// pode ser 1 flag de 2 bytes ou 2 flag de 1 byte
		...
		struct MemorialShopCoinItem {
			...
			unsigned int  filter[4];		// Filtros com os tipos dos item para pesquisar no outro IFF do memorial shop rare item 
			char s_string[24];
		...
		struct MemorialShopRareItem {
			...
			unsigned int  filter[5];		// O mesmo de cima
			char s_string[24];
			std::string toString() {
				return "Typeid: " + std::to_string(_typeid) + "(0x" + hex_util::ltoaToHex(_typeid) + ")"
		...
		struct ScratchRewardSetting {
			...
			unsigned int  _typeid;
			unsigned int  qntd;			// Qntd de itens
			struct Reward {
