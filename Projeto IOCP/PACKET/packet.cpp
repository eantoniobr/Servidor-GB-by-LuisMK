...
void packet::addBuffer(std::vector<unsigned char> buffer)
{
	auto new_buffer = new unsigned char[buffer.size()];
	std::copy(buffer.begin(), buffer.end(), new_buffer);

	add_plain(new_buffer, buffer.size());
}; 
...
std::string packet::readString(int16_t len) {
...
void packet::log()
{
	//_smp::message_pool::getInstance().push(new message("[packet::Log][Hex]:\n\r" + hex_util::BufferToHexString(getBuffer(),getSize()), CL_ONLY_CONSOLE));
};