...
void MemorialSystem::initialize() {
	...
	{
		MEMORIAL_RARE_TYPE type = MEMORIAL_RARE_TYPE(i);
		switch (type)
		{
		case COOKIES:
			m_rate[type] = ctx_rate{ type, 100 };
			break;
		case RARE:
			m_rate[type] = ctx_rate{ type, 20 };
			break;
		case INCOMUM:
			m_rate[type] = ctx_rate{ type, 15 };
			break;
		case SUPER_RARE:
			m_rate[type] = ctx_rate{ type, 10 };
			break;
		case SUPER_RARE2:
			m_rate[type] = ctx_rate{ type, 5 };
			break;
		default:
			break;
		}
...
ctx_rate* MemorialSystem::findRate(MEMORIAL_RARE_TYPE _typeid) {

	TRY_CHECK;

	auto it = m_rate.find(_typeid);
...
std::vector< ctx_coin_item_ex > MemorialSystem::drawCoin(player& _session, ctx_coin& _ctx_c) {
	...
	bool event_x2 = (sgs::gs::getInstance().getInfo().rate.memorial_shop) / 100 >= 2;
	if (event_x2)
	{
		...
		for (auto& el : _ctx_c.item) {
			...
			{
				//_smp::message_pool::getInstance().push(new message("[MemorialSystem::drawCoin][WARNING] ja tem esse item[TYPEID=" + std::to_string(el._typeid) + "] na conta.",CL_ONLY_CONSOLE));	
			}
			else
			{
				// Pega o Rate do Game Server
				//el.probabilidade = findRate(el.tipo)->probabilidade;
				...
				case MCT_SPECIAL:	// Special n�o tem limite de level, ele pega todos
					lottery.push(el.probabilidade, (size_t)&el);
					break;
				case MCT_CHARACTER:	// Special n�o tem limite de level, ele pega todos
					lottery.push(el.probabilidade, (size_t)&el);
					break;
				default:
	...
	auto rate_memorial = (float)(sgs::gs::getInstance().getInfo().rate.memorial_shop) / 100.f * level;

	// Rate A+ da Coin
	if (event_x2)
	{
		rate_memorial += (float)((_ctx_c.probabilidade == 0? 50: _ctx_c.probabilidade) * 4 / 100.f);	// 100 * 4 / 100 4 / 4 50% coin premium 
	}

	limit_prob = (uint64_t)(limit_prob * (2 / rate_memorial));	// Padr�o 75% do limite de probabilidade consolo, 25% normal
	...
	for (auto& el : m_consolo_premio)
		...
		lottery.push(count_item, (size_t)&el.second);
