...
#include "../PANGYA_DB/cmd_coupon_desconto.hpp"
#include "../PANGYA_DB/cmd_grand_zodiac_pontos.hpp"
#include "../PANGYA_DB/cmd_insert_scratch_card_rare_win_log.hpp"
#include "../PANGYA_DB/cmd_change_nick.hpp"
	...
	if (!_session.m_pi.m_cap.stBit.game_master) {
		...
		if (_session.m_pi.level < 0 || _session.m_pi.level > 70)
			throw exception("[channel::checkEnterChannel][Error] Player[UID=" + std::to_string(_session.m_pi.uid) + ", LEVEL=" + std::to_string(_session.m_pi.level)
				+ "] nao tem o level necessario para entrar no canal[ID=" + std::to_string((unsigned short)m_ci.id) + ", MIN=" + std::to_string(0)
				+ ", MAX=" + std::to_string(70) + "].", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 3));

		if (m_ci.flag.stBit.only_rookie && _session.m_pi.level > PlayerInfo::enLEVEL::ROOKIE_A)
			throw exception("[channel::checkEnterChannel][Error] Player[UID=" + std::to_string(_session.m_pi.uid) + ", LEVEL=" + std::to_string(_session.m_pi.level)
				+ "] nao tem o level necessario para entrar no canal[ID=" + std::to_string((unsigned short)m_ci.id) + ", MIN=" + std::to_string(0)
				+ ", MAX=" + std::to_string(70) + "] com a flag So Rookie.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 3));

		if (m_ci.flag.stBit.junior_bellow && _session.m_pi.level > PlayerInfo::enLEVEL::JUNIOR_A)
			throw exception("[channel::checkEnterChannel][Error] Player[UID=" + std::to_string(_session.m_pi.uid) + ", LEVEL=" + std::to_string(_session.m_pi.level)
				+ "] nao tem o level necessario para entrar no canal[ID=" + std::to_string((unsigned short)m_ci.id) + ", MIN=" + std::to_string(0)
				+ ", MAX=" + std::to_string(70) + "] com a flag Junior A pra baixo.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 3));

		if (m_ci.flag.stBit.junior_above && _session.m_pi.level < PlayerInfo::enLEVEL::JUNIOR_E)
			throw exception("[channel::checkEnterChannel][Error] Player[UID=" + std::to_string(_session.m_pi.uid) + ", LEVEL=" + std::to_string(_session.m_pi.level)
				+ "] nao tem o level necessario para entrar no canal[ID=" + std::to_string((unsigned short)m_ci.id) + ", MIN=" + std::to_string(0)
				+ ", MAX=" + std::to_string(70) + "] com a flag Junior E pra cima.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 3));
		...
		if (m_ci.flag.stBit.junior_between_senior && (_session.m_pi.level < PlayerInfo::enLEVEL::JUNIOR_E || _session.m_pi.level > PlayerInfo::enLEVEL::SENIOR_A))
			throw exception("[channel::checkEnterChannel][Error] Player[UID=" + std::to_string(_session.m_pi.uid) + ", LEVEL=" + std::to_string(_session.m_pi.level)
				+ "] nao tem o level necessario para entrar no canal[ID=" + std::to_string((unsigned short)m_ci.id) + ", MIN=" + std::to_string(0)
				+ ", MAX=" + std::to_string(70) + "] com a flag junior E a Senior A.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 3));
		...
		if (m_ci.flag.stBit.beginner_between_junior && (_session.m_pi.level < PlayerInfo::enLEVEL::BEGINNER_E || _session.m_pi.level > PlayerInfo::enLEVEL::JUNIOR_A))
			throw exception("[channel::checkEnterChannel][Error] Player[UID=" + std::to_string(_session.m_pi.uid) + ", LEVEL=" + std::to_string(_session.m_pi.level)
				+ "] nao tem o level necessario para entrar no canal[ID=" + std::to_string((unsigned short)m_ci.id) + ", MIN=" + std::to_string(0)
				+ ", MAX=" + std::to_string(70) + "] com a flag Beginner E a Junior A.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 3));
	}
};
...
void channel::makeGrandZodiacEventRoom(range_time& _rt) {
	...
	try {
		...
		constexpr char GRAND_ZODIAC_EVENT_INT_NAME[] = "HIO Event (BEGINNER)";
		constexpr char GRAND_ZODIAC_EVENT_ADV_NAME[] = "HIO Event (PRO)";
...
void channel::makeBotGMEventRoom(stRangeTime& _rt, std::vector< stReward > _reward) {
	...
	try {
		...
		if (rooms.empty()) {
			...
			ri.time_30s = 25 * 60000; // 35 min
			ri.tipo = RoomInfo::TIPO::TOURNEY;
			ri.qntd_hole = 9; // 18 Holes
	...
	try {
		...
		if (qntd > 0) {
			...
			if (coupon.id != 0) {
				...
				//criado por luisMK
				CmdCouponDesconto cmd_coupon(true);	// Waiter

				snmdb::NormalManagerDB::getInstance().add(0, &cmd_coupon, nullptr, nullptr);

				cmd_coupon.waitEvent();

				if (cmd_coupon.getException().getCodeError() != 0)
					throw cmd_coupon.getException();

				auto db_coupon = cmd_coupon.getInfo(wi_coupon->_typeid);

				uint64_t old_price = cookie;
				std::string type_desconto = "Coupon de 5%"; // Padrão

				if (db_coupon._typeid != 0)
				{
					// 5 CP de desconto
					if (((int64_t)cookie - db_coupon.valor) < 0)
						cookie = 0ull;
					else
						cookie -= db_coupon.valor;

					type_desconto = db_coupon.Nome;
				}
				else // Padrão para coupons desconhecido pelo server hardcoded
				{
					cookie = (uint64_t)(cookie * 0.95f);
				}
				//------------------ terminou meu codigo aqui
				// Log
				...
				_smp::message_pool::getInstance().push(new message(coupon_msg, CL_ONLY_CONSOLE));
			}
...
void channel::requestOpenBoxMyRoom(player& _session, packet* _packet) {
	...
	try {
		...
		auto box = sBoxSystem::getInstance().findBox(_session.m_pi.uid, pWi->_typeid);
	...
	try {
		...
		if (pIb != nullptr) {	// já tem um equipado aumenta o tempo dele
			...
			if (pIb->_typeid != ib._typeid) {	// Mesmo Efeito mas typeid diferente, renova o tempo, e muda o efeito qntd e tempo
				...
				pIb->end_date = UnixToSystemTime(SystemTimeToUnix(pIb->use_date) + (static_cast<long long>(tli->time) * 60/*FROM MINUTES TO SECONDS*/));
				...
				pIb->end_date = UnixToSystemTime((uint32_t)(new_end_date + (static_cast<long long>(tli->time) * 60/*FROM MINUTES TO SECONDS*/)));
...
void channel::requestPlayBigPapel(player& _session, packet* _packet) {
	REQUEST_BEGIN("requestPlayBigPapelShop");
	...
	try {
		...
		// Verifica se session está autorizada para executar esse ação, 
		// se ele não fez o login com o Server ele não pode fazer nada até que ele faça o login
		CHECK_SESSION_IS_AUTHORIZED("requestPlayBigPapelShop");
		...
		// Add +1 ao contador de jogo ao play Palpel Shop
		sys_achieve.incrementCounter(0x6C40004Au, 10/*Play Papel Shop*/);
...
void channel::requestOpenScratchCard(player& _session, packet* _packet)
{
	REQUEST_BEGIN("requestOpenScratchCard");
	packet p;
	...
	try {
		...
		if (_session.m_pi.block_flag.m_flag.stBit.scratchy)
			throw exception("[game_server::requestOpenScratchCard][Error] Player[UID=" + std::to_string(_session.m_pi.uid)
				+ "] esta bloqueado o Rank Server, ele nao pode acessar o requestOpenScratchCard.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 7010, 0));


		p.init_plain((unsigned short)0x1EB);
		...
		_smp::message_pool::getInstance().push(new message("[game_server::requestOpenScratchCard][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
...
void channel::requestPlayScratchCard(player& _session, packet* _packet)
{
	...
	try {
		...
		if (coupon != nullptr) {
			...
			_smp::message_pool::getInstance().push(new message("[channel::requestPlayScratchCard][COUPON_QTND] " + std::to_string(qtnd), CL_FILE_LOG_AND_CONSOLE));

			// Add ao vector(no codigo do acrisio us nao coloca os cards
			//v_item.push_back(item);
		...
		auto rare = std::for_each(balls.begin(), balls.end(), [&](auto& el) {
			if (el.ctx_psi.tipo == PST_RARE) {
				_smp::message_pool::getInstance().push(new message("[ScratchCardSystem::PlayNormal][Log] player[UID=" + std::to_string(_session.m_pi.uid) + "] ganhou Item Raro[TYPEID="
					+ std::to_string(el.ctx_psi._typeid) + ", QNTD=" + std::to_string(el.qntd) + ", BALL_COLOR=" + "0" + ", PROBABILIDADE=" + std::to_string(el.ctx_psi.probabilidade) + "]", CL_FILE_LOG_AND_CONSOLE));

				// Add +1 ao contador de item Rare Win no Scratch Card
				snmdb::NormalManagerDB::getInstance().add(30, new CmdInsertScratchCardRareWinLog(_session.m_pi.uid, el), channel::SQLDBResponse, this);
			}
			});


		// Log
		_smp::message_pool::getInstance().push(new message("[ScratchCardSystem::PlayNormal][Log] player[UID=" + std::to_string(_session.m_pi.uid) + "] jogou Scratch Card Normal e ganhou Item(ns){" + ids + "}", CL_FILE_LOG_AND_CONSOLE));

		p.init_plain((unsigned short)0xD5);
		p.addUint32(coupon->id);		// OK
		packet_func::session_send(p, &_session, 1);
		...
		p.addUint32(0);		// OK
		p.addUint32((uint32_t)balls.size());		// OK
		for (auto& el : balls) {
			p.addUint32(0);		// OK
			p.addUint32(el.ctx_psi._typeid);
			p.addInt32((el.item != 0) ? ((stItem*)el.item)->id : 0);	// Precisa do ID, se não ele add 2 itens, o do pacote 216 e o desse
		...
		_smp::message_pool::getInstance().push(new message("[channel::requestPlayScratchCard][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
...
void stdA::channel::requestPlayScratchCardSerial(player& _session, packet* _packet)
{
	...
	try {
		_smp::message_pool::getInstance().push(new message("[channel::requestPlayScratchCardSerial] Hex:\n\r" + hex_util::BufferToHexString(_packet->getBuffer(), _packet->getSize()), CL_FILE_LOG_AND_CONSOLE));
		...
		if (_session.m_pi.block_flag.m_flag.stBit.scratchy)
			throw exception("[channel::requestPlayScratchCardSerial][Error_Block] player[UID=" + std::to_string(_session.m_pi.uid)
				+ "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 0x790001));

		if (scard_info.Check_Size < 13)
			throw exception("[channel::requestPlayScratchCardSerial][Error_Check_Size] player[UID=" + std::to_string(_session.m_pi.uid)
				+ "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 0x790001));

		scard_info.Serial = _packet->readString(scard_info.Check_Size);

		if (scard_info.Serial.empty())
			throw exception("[channel::requestPlayScratchCardSerial][Error_String] player[UID=" + std::to_string(_session.m_pi.uid)
				+ "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 0x790001));


		if (scard_info.Serial.size() != scard_info.Check_Size)
			throw exception("[channel::requestPlayScratchCardSerial][Error_Size] player[UID=" + std::to_string(_session.m_pi.uid)
				+ "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 3, 0x790001));

		// Resposta para o Play Scratch Card Serial
		p.init_plain((unsigned short)0x00DE);
		p.addUint32(0);		// OK
		p.addUint32((uint32_t)3);// OK,return code 0 success, 1 used, 2 invalid, 3 expired etc...
		p.addUint32(0);
		packet_func::session_send(p, &_session, 1);
		...
		_smp::message_pool::getInstance().push(new message("[channel::requestPlayScratchCardSerial][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
...
void channel::requestChangeNickName(player& _session, packet* _packet)
{
	REQUEST_BEGIN("PlayerChangeNickName");

	_smp::message_pool::getInstance().push(new message("Packet 0x050.\n\rHex Dump.\n\r" + hex_util::BufferToHexString((unsigned char*)_packet->getPlainBuf().buf, _packet->getPlainBuf().len), CL_FILE_LOG_AND_CONSOLE));
	...
	try {
		...
		auto new_nick = _packet->readString();

		if ((uint32_t)new_nick.size() < 4 || (uint32_t)new_nick.size() > 16)
		{
			code = 4;
			throw exception(" tamanho do nick invalido, maior que 16 ou menor que 4", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 1, 0));

		}

		if (_session.m_pi.cookie < 7000 && _session.m_pi.mi.capability.stBit.game_master == 0)
		{
			code = 5;
			throw exception(" jogador nao tem 7000 cookies para trocar o nick", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 1, 0));

		}
		if (_session.m_pi.cookie < 7000 && _session.m_pi.mi.capability.stBit.game_master)
		{
		...
		snmdb::NormalManagerDB::getInstance().add(0, &cmd_change_nick, nullptr, nullptr);
		...
		code = cmd_change_nick.getCode();
		if (code != 1 && _session.m_pi.mi.capability.stBit.game_master == 0)
		{
			code = 7;
			throw exception(" error code 2 or 0 ", STDA_MAKE_ERROR(STDA_ERROR_TYPE::CHANNEL, 1, 0));
		}
		if (code == 1)
		{
		...
		p.addUint32(code);
		packet_func::session_send(p, &_session, 0);
		_smp::message_pool::getInstance().push(new message("[Channel::PlayerChangeNickName][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
};
...
//#endif // _RELEASE
		...
		// Fim do teste
		int code = CommandByChat(_session, std::stringstream(_msg));
...
int channel::CommandByChat(player& _session, std::stringstream command)
{
	...
	{
		if (cmd.compare("@gmevent") == 0 && r == nullptr && _session.m_pi.mi.sala_numero == -1)
		{
			code = 1;
			...
			ri.modo = RoomInfo::MODO::M_SHUFFLE;
			ri.master = -2;
			ri.natural.ulNaturalAndShortGame = 0;	// Short Game e Natural está nessa flag também
			...
			p.addString(" \\c0xff00ff00\\cCreate Auto Room by GM Sucess !");
			packet_func::session_send(p, &_session, 1);


			sgs::gs::getInstance().requestSendNotice(std::string("[NOTICIA]: GM Evento com 9 holes ira comeca, chame seus amigos e participe !"));
		}

		if (cmd.compare("@bot_add") == 0 && r != nullptr && r->getNumPlayers() == 1)
		{
			code = 1;
			// Add Bot Tourney Visual para a sala
			r->addBotVisual(_session);
			...
			p.addString("[COMMAND_CHAT]");
			p.addString("Bot Criado com sucesso !");
			packet_func::session_send(p, &_session, 1);
		...
		if (cmd.compare("@autoplay") == 0 && r != nullptr && r->getNumPlayers() > 1)
		{
			...
			p.addString("[COMMAND_CHAT]");
			p.addString("c0xff00ff00\\cSala Iniciada Com Sucesso !");
			packet_func::session_send(p, &_session, 1);
		}
		...
		if (cmd.compare("@wind") == 0 && r != nullptr)
		{
			...
			std::string cmd1 = "";//valor

			uint16_t wind = 0;
			uint16_t degree = 0;
			command >> wind;
			command >> cmd1;
			if (cmd1.compare("@degree"))
			{
				command >> degree;
			}
			...
			p.addString("[COMMAND_CHAT]");
			p.addString(" \\c0xff00ff00\\cChange Wind Sucess !");
			packet_func::session_send(p, &_session, 1);
		}
		if (cmd.compare("@sendpangs") == 0)
		{
			code = 1;

			uint64_t pang = 0;
			command >> pang;
			if (pang <= 0 || pang >= 999999)
			{
				code = 0;
			}
			else
			{
				...
				p.addString("[COMMAND_CHAT]");
				p.addString(" \\c0xff00ff00\\cSend Gift Pangs by GM Sucess !");
				for (auto& _el : v_sessions)
				{
					if (_el->m_pi.uid != _session.m_pi.uid)
					{
						_el->addPang(pang);
						packet_func::session_send(p, _el, 1);
					}
		...
		if (cmd.compare("@sendcookies") == 0)
		{
			code = 1;

			uint64_t cookies = 0;
			command >> cookies;
			if (cookies <= 0 || cookies >= 999999)
			{
				code = 0;
			}
			else
			{
				...
				p.addString("[COMMAND_CHAT]");
				p.addString(" \\c0xff00ff00\\cSend Gift Cookies by GM Sucess !");
				for (auto& _el : v_sessions)
				{
					if (_el->m_pi.uid != _session.m_pi.uid)
					{
						_el->addCookie(cookies);
						packet_func::session_send(p, _el, 1);
					}
...
void channel::addSession(player* _session) {
	...
	if (_session->m_pi.level >= 6 && _session->m_pi.ui.jogado >= 50) {
		...
		if (rate < GOOD_PLAYER_ICON)
			_session->m_pi.mi.state_flag.stFlagBit.azinha = 0;
...
#if defined(_WIN32)
	memcpy_s(pci.guild_mark_img, sizeof(pci.guild_mark_img), _session.m_pi.gi.Image, sizeof(pci.guild_mark_img));
#elif defined(__linux__)
	memcpy(pci.guild_mark_img, _session.m_pi.gi.mark_emblem, sizeof(pci.guild_mark_img));
#endif
	//pci.unknown106
	std::string nickNT("@");
	nickNT += std::string(pci.nickname);
	STRCPY_TO_MEMORY_FIXED_SIZE(pci.nickNT, sizeof(pci.nickNT), nickNT.c_str());
...
void channel::SQLDBResponse(uint32_t _msg_id, pangya_db& _pangya_db, void* _arg) {
	...
	case 29:
	{auto cmd_ipsrwl = reinterpret_cast<CmdInsertScratchCardRareWinLog*>(&_pangya_db);

	_smp::message_pool::getInstance().push(new message("[channel::SQLDBResponse][Log] player[UID=" + std::to_string(cmd_ipsrwl->getUID()) + "] Adicionou Papel Shop Rare Win Log[TYPEID="
		+ std::to_string(cmd_ipsrwl->getInfo().ctx_psi._typeid) + ", QNTD=" + std::to_string(cmd_ipsrwl->getInfo().qntd) + ", PROBABILIDADE=" + std::to_string(cmd_ipsrwl->getInfo().ctx_psi.probabilidade) + "]", CL_FILE_LOG_AND_CONSOLE));
	break; }
	case 0:
