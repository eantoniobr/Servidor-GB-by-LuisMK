...
void AttendanceRewardSystem::sendScratchCardTicket(player& _session) {
	CHECK_SESSION("sendScratchTicket");

	TRY_CHECK;

	auto pWi = _session.m_pi.findWarehouseItemByTypeid(436207779);

	// Envia os 3 Grand Prix para o player, ele n�o tem nenhum ticket ou n�o atingiu o limite
	if (pWi == nullptr || pWi->STDA_C_ITEM_QNTD < 4) {

		stItem item{ 0 };

		item.type = 2;
		item.id = -1;
		item._typeid = 436207779;
		item.qntd = (pWi == nullptr) ? 3 : ((3 - pWi->STDA_C_ITEM_QNTD) >= 3 ? 3 : 3 - pWi->STDA_C_ITEM_QNTD);
		item.STDA_C_ITEM_QNTD = (short)item.qntd;
		...
		if ((rt = item_manager::addItem(item, _session, 0, 0)) < 0/*Error*/)
			throw exception("[AttendanceRewardSystem::sendScratchTicket][Error] player[UID=" + std::to_string(_session.m_pi.uid)
				+ "] tentou adicionar o ScractchCard Ticket do login, mas nao conseguiu. Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::ATTENDANCE_REWARD_SYSTEM, 9, 0));

		// Log
		_smp::message_pool::getInstance().push(new message("[AttendanceRewardSystem::sendScratchTicket][Log] player[UID=" + std::to_string(_session.m_pi.uid)
			+ "] enviou os ScractchCard Ticket para ele com sucesso.", CL_FILE_LOG_AND_CONSOLE));
		...
		_smp::message_pool::getInstance().push(new message("[AttendanceRewardSystem::sendScratchTicket][Log] player[UID=" + std::to_string(_session.m_pi.uid) + "] ja tem o limite[LIMIT="
			+ std::to_string(3) + "] de Scratch Card Ticket.", CL_ONLY_FILE_LOG_TEST));
