...
//anel e outros itens que podem aumentar ou duplicar o rate
uint32_t luck_item[]{ 1879048197 };
...
unsigned char stdA::Game::checkLuckItem(player& _session)
{
	// Characters Equip
	if (_session.getState() &&
#if defined(_WIN32)
		...
		if (_session.m_pi.ei.char_info == nullptr) {	// Player não está com character equipado, kika dele do jogo
			_smp::message_pool::getInstance().push(new message("[Game::checkLuckItem][Log] player[UID=" + std::to_string(_session.m_pi.uid) + "] nao esta com Character equipado. kika ele do jogo. pode ser Bug.",
				CL_FILE_LOG_AND_CONSOLE));
		...
		// Motion Item
		if (FIND_ELEMENT_ARRAY_OF_ARRAY(_session.m_pi.ei.char_info->auxparts, luck_item)) {
#ifdef _RELEASE
			_smp::message_pool::getInstance().push(new message("[Game::checkLuckItem][Log] player[UID=" + std::to_string(_session.m_pi.uid) + "] esta equipado com Motion Item no Character[TYPEID="
				+ std::to_string(_session.m_pi.ei.char_info->_typeid) + ", ID=" + std::to_string(_session.m_pi.ei.char_info->id) + "]", CL_FILE_LOG_AND_CONSOLE));
#endif // _RELEASE
			return 1u;
		}
...
void Game::makePlayerInfo(player& _session) {
	...
	pgi->char_aux_item = checkLuckItem(_session);//anel de sorte
