...
#include "../PANGYA_DB/cmd_box_rare_item.hpp"
...
ctx_box* BoxSystem::findBox(uint32_t uid, uint32_t _typeid)
{
	if (!m_box.empty())
		m_box.clear();
	// Carrega as box do banco de dados
	CmdBoxRareItem cmd_bi(uid, _typeid, CmdBoxRareItem::NEW, true);	// Waiter
	snmdb::NormalManagerDB::getInstance().add(0, &cmd_bi, nullptr, nullptr);
