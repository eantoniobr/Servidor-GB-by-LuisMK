...
bool room::isGMEvent()
{
	return m_ri.flag_gm == 0x100 && getNumPlayers() > 99;//por enquanto eu so pego o flag aqui
};
...
#if defined(_WIN32)
	memcpy_s(pri.guild_mark_img, sizeof(pri.guild_mark_img), _session.m_pi.gi.Image, sizeof(pri.guild_mark_img));
#elif defined(__linux__)
	...
	try {
		...
		for (auto i = 0u; i < num_info; ++i) {
			...
			case RoomInfo::INFO_CHANGE::SENHA:
				...
				if (isLocked() && checkPass("bot") && getNumPlayers() == 1)
				{
					makeBot(_session);
				}
				break;
...
PlayerRoomInfoEx* room::makePlayerInfo(player & _session) {
	...
	std::string nickNT("@");
	nickNT += std::string(pri.nickname);
	STRCPY_TO_MEMORY_FIXED_SIZE(pri.nickNT, sizeof(pri.nickNT), nickNT.c_str());
...
void room::updateGuild(player & _session) {
	...
	if (m_ri.guilds.guild_1_uid == 0 && m_ri.guilds.guild_2_uid != _session.m_pi.gi.uid) {
		...
		memcpy_s(m_ri.guilds.guild_1_mark, sizeof(m_ri.guilds.guild_1_mark), _session.m_pi.gi.Image, sizeof(m_ri.guilds.guild_1_mark));
#elif defined(__linux__)
		...
		memcpy_s(m_ri.guilds.guild_2_mark, sizeof(m_ri.guilds.guild_2_mark), _session.m_pi.gi.Image, sizeof(m_ri.guilds.guild_2_mark));
#elif defined(__linux__)
...
void room::addBotVisual(player & _session) {
	...
	try {
		...
		pri.title = 0;		// Title Helper
#if defined(_WIN32)
		strcpy_s(pri.nickname, "\\1NEXAGAMES(e7)");
#elif defined(__linux__)
		strcpy(pri.nickname, "\\1NEXAGAMES(e7)");
#endif
