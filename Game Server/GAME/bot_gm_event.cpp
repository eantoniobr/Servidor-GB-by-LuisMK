...
void BotGMEvent::initialize() {
	...
	TRY_CHECK;
		...
		if (m_rewards.size() <=0 || m_rt.size() <=0)
		{
			_smp::message_pool::getInstance().push(new message("[BotGMEvent::initialize][Log] Bot GM Event System nao carregado com sucesso!", CL_FILE_LOG_AND_CONSOLE));
		}
