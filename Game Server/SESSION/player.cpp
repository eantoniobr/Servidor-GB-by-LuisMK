...
void player::ReloadGuildInfo()
{

	CmdGuildInfo guild_list(m_pi.uid, 1, true);	// Waiter
	...
	m_pi.gi = guild_list.getInfo();

}

void player::SendGuildData()
{
	try {
		...
		p.init_plain((unsigned short)0x1BF);//ew byte[]{ 0xBF, 0x01 };
		p.addInt32(1);
		p.addInt32(m_pi.gi.uid); // Guild ID
		p.addFixedString(m_pi.gi.name, 17);
		p.addInt32(m_pi.gi.point);
		p.addInt32(m_pi.gi.pang);
		p.addUint8(0);
		p.addInt32(m_pi.gi.total_member);
		p.addFixedString(std::string(m_pi.gi.Image), 12);
		p.addFixedString(std::string(m_pi.gi.Notice), 101);
		p.addFixedString(std::string(m_pi.gi.Introducting), 101);
		p.addInt32(m_pi.gi.Position);
		p.addInt32(m_pi.gi.LeaderUID);
		p.addFixedString(m_pi.gi.LeaderNickname, 22);
