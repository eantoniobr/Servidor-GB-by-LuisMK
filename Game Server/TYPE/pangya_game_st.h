...
#pragma pack(1)
	...
	union uMemberInfoStateFlag {
		void clear() {
			memset(this, 0, sizeof(uMemberInfoStateFlag));
			/*stFlagBit.visible = 0;
			stFlagBit.whisper = 0;
			stFlagBit.channel = 0;
			stFlagBit.sexo = 0;
			stFlagBit.azinha = 0;
			stFlagBit.quiter_1 = 0;
			stFlagBit.quiter_2 = 0;
			stFlagBit.icon_angel = 0;*/
		};
	...
	struct MemberInfo {
		...
		char unk[16];
		char nickNT[18];						// ainda não sei direito o que tem aqui
		unsigned char ucUnknown107[110];	// ainda não sei direito o que tem aqui
	};
	...
	struct UserInfo {
		...
		std::string toString() {
			return "Tacada: " + std::to_string(tacada) + "  Putt: " + std::to_string(putt) + "  Tempo: " + std::to_string(tempo) + "  Tempo Tacada: " + std::to_string(tempo_tacada)
				...
				+ "  Disconnect(MY): " + std::to_string(0) + "  Jogados Disconnect(MY): " + std::to_string(jogados_disconnect) + "  Event value: " + std::to_string(event_value)
				+ "  Skin Strike Point: " + std::to_string(skin_strike_point) + "  Sistema School Serie: " + std::to_string(0)
...
#define STDA_C_ITEM_TIME c[3]
	...
	struct PlayerCanalInfo {
		...
		unsigned char flag_visible_gm;//th é vip
		char unknown[6];
		char nickNT[18];				// Acho
		char unknown109[109];
	};
	...
	// Player Canal Info
	struct PlayerCanalInfoEx :public PlayerCanalInfo {
		PlayerCanalInfoEx() : PlayerCanalInfo()
		{
			PlayerCanalInfo::clear();
		}
		unsigned char flag_visible_gm;
	};
	// Player Room Info {
	struct PlayerRoomInfo {
		...
		uint32_t uid;
		//---------Action
		uint32_t state_lounge;//animate
		unsigned short usUnknown_flg;//Unknown1	// Acho que seja uma flag tbm
		uint32_t state;//Posture	// Acho que seja estado de "lugar" pelo que lembro
		struct stLocation {
		...
		uItemBoost flag_item_boost;// Boost EXP, Pang e etc(2 bytes)
		uint32_t ulUnknown_flg;// Pode ser a flag de teasure do player, ou de drop item
		unsigned char nickNT[22];// Acho que seja o ID na ntreev do player, a empresa que mantêm as contas, no JP era o gamepot
		unsigned char ucUnknown105[105];// Unknown 106 bytes que sempre fica com valor 0
		...
		float ucUnknown3;// Não sei mas sempre é 0 depois do media score(66 no th)
	};
	...
	struct RoomGuildInfo {
		...
		char guild_1_nome[17];
		char guild_2_nome[17];
	...
	struct ClientVersion {
		...
		std::string fixedValue(uint32_t _value, uint32_t _width) {
			...
			results << std::setw((_value < 0 ? static_cast<std::streamsize>(_width) + 1 : _width)) << _value;
