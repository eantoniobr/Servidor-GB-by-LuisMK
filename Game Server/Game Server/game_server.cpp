...
#include "../GAME/scratch_card_system.hpp"
...
#include "../PANGYA_DB/cmd_member_logon.hpp"
...
#include "../PANGYA_DB/cmd_insert_notice.hpp"
...
game_server::game_server() : server(m_player_manager, 2, 26, 8), m_player_manager(*this, m_si.max_user), m_dqi{0}, m_login_manager(), m_timer_manager(),
	...
	try {
		...
		config_init();
		ConsoleEx cmd;
		std::string name(m_si.nome);
		cmd.ConsoleSetTitle("Game Server: " + name + " - " + " PLAYERS: " + std::to_string(0));
...
#error Unknown pointer size or missing size macros!
		...
		snmdb::NormalManagerDB::getInstance().add(13, new CmdMemberLogon(m_si.uid, false), game_server::SQLDBResponse, this);
...
void game_server::requestLogin(player& _session, packet *_packet) {
	...
	try {
		...
		if (haveBanList(_session.m_ip, mac_address, !mac_address.empty()))
		...
		*(player_info*)pi = cmd_pi.getInfo();
		...
		snmdb::NormalManagerDB::getInstance().add(11, new CmdFirstAnniversary(), game_server::SQLDBResponse, this);
	...
	try {
		...
		if (c != nullptr) {
			...
			if (_session.m_pi.mi.sala_numero != -1)
				...
				int code = c->CommandByChat(_session, std::stringstream(msg));
...
void game_server::requestSendNotice(std::string msg)
{

	// Add o Ticker para lista de ticker do server
	m_notice.push_back((int)std::time(nullptr) + 10/*Segundos*/,  msg, BroadcastList::TYPE::GM_NOTICE);


	// Add o Ticker para Commando DB para o Auth Server mandar para os outros serveres
	snmdb::NormalManagerDB::getInstance().add(12, new CmdInsertNotice(m_si.uid, msg), game_server::SQLDBResponse, this);

	
	_smp::message_pool::getInstance().push(new message("[game_server::requestSendNotice][Log] BroadCast Notice: " + hex_util::StringToHexString(msg), CL_FILE_LOG_AND_CONSOLE));
...
void game_server::requestSendTicker(player& _session, packet *_packet) {
	...
	try {
		...
		try {
			...
			cp_log.setCookie(300);
			// fim do inicializa log de gastos de CP

			_session.m_pi.consomeCookie(300);
...
#error Unknown pointer size or missing size macros!
	...
	//SHOW_RELEASE_FINAL_PACKET(mb, 1);
//gs.start();
	...
	try {
		...
		ConsoleEx cmd;
		std::string name(m_si.nome);
		cmd.ConsoleSetTitle("Game Server: " + name + " - " + " PLAYERS: " + std::to_string(m_si.curr_user));
...
bool game_server::checkCommand(std::stringstream& _command) {
	...
	else if (!s.empty() && s.compare("cls") == 0)
	{
	system("cls");
	}
	...
	else if (!s.empty() && s.compare("up_coin_loc") == 0) { // !@ Teste
...
bool game_server::checkPacket(session& _session, packet* _packet) {
	...
	if (_session.m_check_packet.checkPacketId(_packet->getTipo())) {
		...
		case 0x12D: // Pacote de reposta de valores iniciais de camera Inicial do Grand Zodiac, o server manda 10 valores ele manda os 10 de volta em resposta, aumenta para 10 o limite
			...
			break;
		case 0x8B:
			limit_count += ( _session.m_check_packet.incrementCount() - limit_count);
			break;
		case 0x12E:	// Aumenta mais 20, no pacote de marcar no course, para não da dc, muito rápido
			limit_count += 20;
			break;
		default:
			limit_count += 2;	// Sobe + para 7 por que o game server é mais difícil de atacar ele
		}
...
void game_server::config_init() {
	...
	m_si.rate.grand_zodiac_event_time = (m_reader_ini.readInt("SERVERINFO", "GZ_EVENT") >= 1 ? true : false);// Ativo por padrão
	m_si.rate.grand_prix_event = (m_reader_ini.readInt("SERVERINFO", "GP_EVENT") >= 1 ? true : false);// Ativo por padrão
	m_si.rate.golden_time_event = (m_reader_ini.readInt("SERVERINFO", "GOLDEN_TIME_EVENT") >= 1 ? true : false);// Ativo por padrão
	m_si.rate.login_reward_event = (m_reader_ini.readInt("SERVERINFO", "LOGIN_REWARD") >= 1 ? true : false);// Ativo por padrão
	m_si.rate.bot_gm_event = (m_reader_ini.readInt("SERVERINFO", "BOT_GM_EVENT") >= 1 ? true : false);// Ativo por padrão
	m_si.rate.smart_calculator = (/*m_reader_ini.readInt("SERVERINFO", "SMART_CALC") >= 1 ? true :*/ false);// Atibo por padrão
	m_si.rate.angel_event = (m_reader_ini.readInt("SERVERINFO", "ANGEL_EVENT") >= 1 ? true : false);// Atibo por padrão
	try {
...
void game_server::init_systems() {
	...
	if (m_si.rate.grand_zodiac_event_time && !sGrandZodiacEvent::getInstance().isLoad())
		sGrandZodiacEvent::getInstance().load();
	...
	if (m_si.rate.golden_time_event && !sGoldenTimeSystem::getInstance().isLoad())
		sGoldenTimeSystem::getInstance().load();

	// Carrega Login Reward System
	if (m_si.rate.login_reward_event && !sLoginRewardSystem::getInstance().isLoad())
		sLoginRewardSystem::getInstance().load();

	// Carrega Bot GM Event
	if (m_si.rate.bot_gm_event && !sBotGMEvent::getInstance().isLoad())
...
void game_server::init_packets()
{
	...
	packet_func::funcs.addPacketCall(0x38, packet_func::packet0050, this);//PlayerChangeNickName
	...
	packet_func::funcs.addPacketCall(0x8B, packet_func::packet08B, this);//msg server
	...
	//---------------------- GUILD SYSTEM --------------------------//
	packet_func::funcs.addPacketCall(0x12A, packet_func::packet1EB, this);
	packet_func::funcs.addPacketCall(0x108, packet_func::packet01BC, this);
	packet_func::funcs.addPacketCall(0x109, packet_func::packet01BD, this);
	packet_func::funcs.addPacketCall(0x10A, packet_func::packet01BE, this);
	packet_func::funcs.addPacketCall(0x104, packet_func::packet01B8, this);
	packet_func::funcs.addPacketCall(0x112, packet_func::packet01C6, this);
	packet_func::funcs.addPacketCall(0x110, packet_func::packet01C4, this);
	packet_func::funcs.addPacketCall(0x106, packet_func::packet01B9, this);
	packet_func::funcs.addPacketCall(0x105, packet_func::packet01BA, this);
	packet_func::funcs.addPacketCall(0x10C, packet_func::packet01C0, this);
	packet_func::funcs.addPacketCall(0x10E, packet_func::packet01D1, this);
	packet_func::funcs.addPacketCall(0x114, packet_func::packet01D1_2, this);
	packet_func::funcs.addPacketCall(0x111, packet_func::packet01C5, this);
	packet_func::funcs.addPacketCall(0x115, packet_func::packet01C9, this);
	packet_func::funcs.addPacketCall(0x116, packet_func::packet01CA, this);
	packet_func::funcs.addPacketCall(0x10D, packet_func::packet01C1, this);
	//----------------------END --------------------------//
	...
	packet_func::funcs.addPacketCall(0x14B, packet_func::packet14B, this);// papel shop normal
	...
	packet_func::funcs.addPacketCall(0x070, packet_func::packet00DD, this);
	packet_func::funcs.addPacketCall(0x071, packet_func::packet00DE, this);

	//somente respostas enviadas pelo addPacketCall de cima
...
void game_server::init_load_channels()
{
	ChannelInfo ci{ 0 };
	...
	for (auto i = 0; i < num_channel; ++i) {
		...
		try {
			...
			if (!STDA_ERROR_CHECK_SOURCE_AND_ERROR(e.getCodeError(), STDA_ERROR_TYPE::READER_INI, 6))
				_smp::message_pool::getInstance().push(new message("[game_server::init_load_channels][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
		}
...
void game_server::reloadGlobalSystem(uint32_t _tipo) {
	...
	try {
		...
		case 18:	// Smart Calculator Lib
			...
			sSmartCalculator::getInstance().load();
			break;
		case 19:	// Scratchy Card System
	// Recarrega Scratchy Card System
			sScratchCardSystem::getInstance().load();
			break;
		default:
...
void game_server::updateRateAndEvent(uint32_t _tipo, uint32_t _qntd) {
	...
	try {
		...
		case 5:	// Scratchy
			...
			// Recarrega o scratchy se ele foi ativado
			if (m_si.rate.scratchy)
				reloadGlobalSystem(19/*Scratchy*/);
			break;
...
void game_server::SQLDBResponse(uint32_t _msg_id, pangya_db& _pangya_db, void* _arg) {
	...
	case 1:	// DailyQuest Info
	{
		...
		if (MgrDailyQuest::checkCurrentQuest(m_dqi)) {
			...
			std::cout << "Daily Quest Atualizada. \n";
		}
		break;
	}
	...
	{
		auto cmd_imo = reinterpret_cast< CmdInsertMsgOff* >(&_pangya_db);	break;
	}
	...
	case 11:
		break;
	case 12:
		reinterpret_cast<CmdInsertNotice*>(&_pangya_db);
		break;

	case 13://nada aqui
		//_smp::message_pool::getInstance().push(new message("[game_server::SQLDBResponse][CmdMemberLogon]  iniciou uma desconexao em massa !", CL_ONLY_CONSOLE));
		break;
	default:
		break;
	}
