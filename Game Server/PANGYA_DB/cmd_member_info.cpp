...
void CmdMemberInfo::lineResult(result_set::ctx_res* _result, uint32_t /*_index_result*/) {
	...
	std::string nickNT("@");
	nickNT += std::string(m_mi.nick_name);
	STRCPY_TO_MEMORY_FIXED_SIZE(m_mi.nickNT, sizeof(m_mi.nickNT), nickNT.c_str());
	...
	if (_result->data[26] != nullptr)
	{
		...
		std::string mark(_result->data[26]);
...
#if defined(_WIN32)
		memcpy_s(m_mi.guild_mark_img, sizeof(m_mi.guild_mark_img), mark.c_str(), mark.size());
#elif defined(__linux__)
		memcpy(m_mi.guild_mark_img, mark.c_str(), mark.c_str(), mark.size());
#endif
	}
	else
	{
#if defined(_WIN32)
		memcpy_s(m_mi.guild_mark_img, sizeof(m_mi.guild_mark_img),std::string("guild_mark").c_str(), sizeof(m_mi.guild_mark_img));
#elif defined(__linux__)
		memcpy(m_mi.guild_mark_img, "guild_mark", sizeof(m_mi.guild_mark_img));
#endif	
	}
