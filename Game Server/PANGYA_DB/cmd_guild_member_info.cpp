...
// Implementa��o da classe CmdGuildMemberInfo

#if defined(_WIN32)
#pragma pack(1)
#endif

#include "cmd_guild_member_info.hpp"

using namespace stdA;

CmdGuildMemberInfo::CmdGuildMemberInfo(bool _waiter) : pangya_db(_waiter), m_uid(0), m_option(0), m_gi{ 0 } {
}

CmdGuildMemberInfo::CmdGuildMemberInfo(uint32_t _uid, uint32_t _option, bool _waiter) : pangya_db(_waiter), m_uid(_uid), m_option(_option), m_gi{ 0 } {
}
...
GuildInfoEx& CmdGuildMemberInfo::getInfo() {
	return m_gi;
}

void CmdGuildMemberInfo::setInfo(GuildInfoEx& _gi) {
	m_gi = _gi;
}

uint32_t CmdGuildMemberInfo::getUID() {
	return m_uid;
}

void CmdGuildMemberInfo::setUID(uint32_t _uid) {
	m_uid = _uid;
}