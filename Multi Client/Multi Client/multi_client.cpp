...
multi_client::multi_client(unsigned long num_connection) : 
	...
	try {
		...
		packet_func::funcs.addPacketCall(0x097, packet_func::packet097, this);
		packet_func::funcs.addPacketCall(0x04A, packet_func::packet04A, this);
		packet_func::funcs.addPacketCall(0x047, packet_func::packet047, this);
		packet_func::funcs.addPacketCall(0x048, packet_func::packet048, this);
		packet_func::funcs.addPacketCall(0x049, packet_func::packet049, this);
		packet_func::funcs.addPacketCall(0x078, packet_func::packet078, this);
		packet_func::funcs.addPacketCall(0x0230, packet_func::packet0230, this);
		packet_func::funcs.addPacketCall(0x0231, packet_func::packet0231, this);
		packet_func::funcs.addPacketCall(0x077, packet_func::packet077, this);
		packet_func::funcs.addPacketCall(0x0196, packet_func::packet0196, this);


		packet_func::funcs_sv.addPacketCall(0x009, packet_func::packet_svFazNada, this);
		packet_func::funcs_sv.addPacketCall(0x008, packet_func::packet_svFazNada, this);
		...
		packet_func::funcs_sv.addPacketCall(0x00D, packet_func::packet_svFazNada, this);
...
void multi_client::start() {
	...
	if (m_num_connection > 0) {
		...
		for (size_t i = 0; i < m_num_connection; ++i) {
			...
			std::string pass("4BADAEE57FED5610012A296273158F5F");
			strcpy_s(ci.m_id, std::string("bot" + std::to_string((i) % 200)).c_str());
			STRCPY_TO_MEMORY_FIXED_SIZE(ci.m_pass, sizeof(ci.m_pass), pass.c_str());
			strcpy_s(ci.m_client_version, "824.00");
			ci.m_packet_version = 2015031200; //2016121900;

			try {
				ConnectAndAssoc("147.135.127.26", 10303, ci);
			}catch (exception& e) {
...
void multi_client::checkClienteOnline() {
	/*for (auto i = 0ul; i < m_cliente_manager.getNumSessionOnline(); ++i) {
		if (m_cliente_manager.getChannelClientes()[i]->m_ci.m_uid == ~0) {
			try {
				ClientInfo ci{};

				ci = m_cliente_manager.getChannelClientes()[i]->m_ci;
				ConnectAndAssoc("147.135.127.26", 10303, ci);
			}catch (exception& e) {
				...
				_smp::message_pool::getInstance().push(new message("Erro desconhecido. multi_client::checkClienteOnline()"));
			}
		}*/
	//}
};
	...
	while (ret == 0) {
		...
		while (!ss.eof()) {
			...
			if (!s.empty() && s.compare("exit") == 0) {
				...
				m_cliente_manager.numSessionConnected();

				auto channel_sessions = m_cliente_manager.getChannelClientes();

				for (auto i = 0ul; i < channel_sessions.size(); ++i) {
					if (channel_sessions[i]->m_ci.m_lobby == 1)
					{
						DisconnectSession(channel_sessions[i]);
					}
				}
			}
