...
int packet_func::packet001(void* _arg1, void* _arg2) {
	...
	if (err == 0) {
		...
		_smp::message_pool::getInstance().push(new message("First Login, enviando os dados para criar um character: " + std::string(pd._session.m_ci.m_id), CL_ONLY_CONSOLE));
	else
...
int packet_func::packet003(void* _arg1, void* _arg2) {
	...
	if (opt == 0) {
		...
		if (ci->m_list_servers.num_servers > 0) {
			...
			STRCPY_TO_MEMORY_FIXED_SIZE(ci_tmp.m_pass, sizeof(ci_tmp.m_pass), pd._session.m_ci.m_pass);
...
int packet_func::packet00F(void* _arg1, void* _arg2) {
	...
	size_t option = pd._packet->readInt8();//conta nova

	if (option ==0) {
		p.init_plain((unsigned short)0x008);
		p.addInt32(67108875);
		p.addInt16(0);
		session_send(p, &pd._session, 1);

		_smp::message_pool::getInstance().push(new message("[packet_func::pacote008] Log: " + std::string(pd._session.m_ci.m_id) + ", Enviando pacote de personagem padrao !", CL_ONLY_CONSOLE));
	}
	else
	{
		_smp::message_pool::getInstance().push(new message("[packet_func::pacote008] Log: " + std::to_string(option), CL_ONLY_CONSOLE));
...
int packet_func::packet011(void* _arg1, void* _arg2) {
	MAKE_BEGIN_PACKET_CLIENT(_arg1, _arg2);
	_smp::message_pool::getInstance().push(new message("Sucesso, estamos entrando no game server agora !", CL_ONLY_CONSOLE));
...
int packet_func::packet044(void* _arg1, void* _arg2) {
	...
	if (err == 0) {
		// Pacote Principal
		_smp::message_pool::getInstance().push(new message("Recebeu Pacote 044 com option 0.", CL_ONLY_CONSOLE));
	}else if (err == 1)
		...
		_smp::message_pool::getInstance().push(new message("Logou no game server com sucesso: " + std::string(pd._session.m_ci.m_id), CL_ONLY_CONSOLE));
	else if (err == 0xD2)
...
int stdA::packet_func::packet097(void* _arg1, void* _arg2)
{
	return 0;
};

int stdA::packet_func::packet0196(void* _arg1, void* _arg2)
{
	return 0;
};
int stdA::packet_func::packet047(void* _arg1, void* _arg2)
{
	MAKE_BEGIN_PACKET_CLIENT(_arg1, _arg2);
	ClientInfo* ci = &pd._session.m_ci;

	ci->m_list_salas.num_salas = pd._packet->readInt8();
	auto option = pd._packet->readInt8();
	auto unk = pd._packet->readInt16();
	//_smp::message_pool::getInstance().push(new message("Packet 0x047.\n\rHex Dump.\n\r" + hex_util::BufferToHexString((unsigned char*)pd._packet->getPlainBuf().buf, pd._packet->getPlainBuf().len), CL_FILE_LOG_AND_CONSOLE));
	if (ci->m_list_salas.num_salas > 0) {
		ci->m_list_salas.a_salas = new ServerRoomInfoEx[ci->m_list_salas.num_salas];

		pd._packet->readBuffer(ci->m_list_salas.a_salas, sizeof(ServerRoomInfo) * ci->m_list_salas.num_salas);

		InterlockedExchange(&ci->m_SC, 1);	// Recebi canais

		_smp::message_pool::getInstance().push(new message("Salas Recebidas: " + std::to_string(ci->m_list_salas.num_salas), CL_ONLY_CONSOLE));
		for (size_t i = 0; i < ci->m_list_salas.num_salas; i++)
		{
			_smp::message_pool::getInstance().push(new message("Sala info: " + (ci->m_list_salas.a_salas[i].result()), CL_ONLY_CONSOLE));
		}
		//ci->m_sluid = ci->m_list_salas.a_salas[0].numero;	// Index 0
		//entraSala(&pd._session, ci->m_sluid);

	}
	return 0;
};
int stdA::packet_func::packet048(void* _arg1, void* _arg2)
{
	return 0;
};
int stdA::packet_func::packet078(void* _arg1, void* _arg2)
{
	return 0;
};
int stdA::packet_func::packet0230(void* _arg1, void* _arg2)
{
	return 0;
};
int stdA::packet_func::packet0231(void* _arg1, void* _arg2)
{
	return 0;
};
int stdA::packet_func::packet077(void* _arg1, void* _arg2)
{
	return 0;
};
...
void packet_func::entraSala(player* _session, unsigned short sala_id, std::string senha) {
...
int packet_func::sendMessage(void *_arg1, void *_arg2) {
	...
	sendMessage(_session, _session->m_ci.m_nickname, "Ola Lobby");
...
int packet_func::packet049(void* _arg1, void* _arg2) {
	MAKE_BEGIN_PACKET_CLIENT(_arg1, _arg2);
	auto tipo = pd._packet->readInt16();
	//_smp::message_pool::getInstance().push(new message("Packet 0x049.\n\rHex Dump.\n\r" + hex_util::BufferToHexString((unsigned char*)pd._packet->getPlainBuf().buf, pd._packet->getPlainBuf().len), CL_FILE_LOG_AND_CONSOLE));
	try
	{
		if (tipo == -1 || tipo == 0)
		{

			auto rom = new ServerRoomInfoEx[0];

			pd._packet->readBuffer(rom, sizeof(ServerRoomInfo));
			
			ClientInfo* ci = &pd._session.m_ci;

			for (size_t i = 0; i < ci->m_list_salas.num_salas; i++)// era pra ter alguma coisa aqui, pra identificar quem esta enviando ne, parece algo sem nexo
			{
				if (rom->numero == ci->m_list_salas.a_salas[i].numero)
				{
					ci->m_list_salas.a_salas[i] = *rom;
				}
			}
			
			if (rom->num_player == rom->max_player || rom->num_player > 1 && (unsigned short)rom->tipo_show != 2)
			{
			...
			_smp::message_pool::getInstance().push(new message("pacote 049: teste !", CL_ONLY_CONSOLE));
		}
		else
		{
			_smp::message_pool::getInstance().push(new message("pacote 049: falha ao entrar na sala com bot cmd !", CL_ONLY_CONSOLE));
		}
	}
...
int packet_func::packet04A(void* _arg1, void* _arg2) {
	...
	//_smp::message_pool::getInstance().push(new message("Packet 0x04A.\n\rHex Dump.\n\r" + hex_util::BufferToHexString((unsigned char*)pd._packet->getPlainBuf().buf, pd._packet->getPlainBuf().len), CL_FILE_LOG_AND_CONSOLE));	
	auto _session = pd._session;
	auto packet = pd._packet;
	try
	{
		auto tipo =packet->readInt16();
		if (tipo == -1 || tipo == 0) {

			auto rom = new ServerRoomInfoEx[0];
			rom->tipo_show = (unsigned char)packet->readInt8();
			rom->course = (ServerRoomInfo::eCOURSE)packet->readInt8();
			rom->qntd_hole = (unsigned char)packet->readInt8();
			rom->modo = (unsigned char)packet->readInt8();
			if (rom->modo == ServerRoomInfo::M_REPEAT) {
				rom->hole_repeat = (unsigned char)packet->readInt8();
				rom->fixed_hole = (unsigned char)packet->readUint32();
			}

			rom->natural.ulNaturalAndShortGame = packet->readUint32();
			rom->max_player = (unsigned char)packet->readInt8();
			rom->_30s = (unsigned char)packet->readInt8();
			rom->state_flag = (unsigned char)packet->readInt8();
			rom->time_vs = (unsigned char)packet->readUint32();
			rom->time_30s = (unsigned char)packet->readUint32();
			rom->trofel = (unsigned char)packet->readUint32();
			rom->senha_flag = (unsigned char)packet->readInt8();
			if (rom->senha_flag)
			{
				rom->senha = packet->readString();
			}
			strcpy_s(rom->nome, packet->readString().c_str());
			ClientInfo* ci = &pd._session.m_ci;

			for (size_t i = 0; i < ci->m_list_salas.num_salas; i++)// era pra ter alguma coisa aqui, pra identificar quem esta enviando ne, parece algo sem nexo
			{
			}
			_smp::message_pool::getInstance().push(new message("pacote 04A: log:" +rom->result() + "\t Senha: " + rom->senha, CL_ONLY_CONSOLE));
		}
	}
	catch (const std::exception& ex)
	{
		_smp::message_pool::getInstance().push(new message("Player UID: " + std::to_string(_session.m_ci.m_uid), CL_FILE_LOG_AND_CONSOLE));
	}
